...
export class PortAllocationError extends Error {
    code;
    availablePorts;
    constructor(message, code, availablePorts) {
        super(message);
        this.code = code;
...
export class PortAllocator {
     ...
     * @returns 分配的端口数组
     */
    async allocatePorts(projectId, count) {
        // 验证参数
        if (count <= 0) {
            throw new PortAllocationError('端口数量必须大于0', 'INVALID_COUNT');
        }
        if (count > portConfig.maxPerProject) {
            throw new PortAllocationError(`单个项目最多只能分配${portConfig.maxPerProject}个端口`, 'EXCEED_MAX_PER_PROJECT');
        }
        // 检查项目是否已经分配过端口
        ...
        if (existingProject.ports) {
            throw new PortAllocationError(`项目"${existingProject.name}"已经分配过端口，无法重复分配`, 'PORTS_ALREADY_ALLOCATED');
        }
        // 使用重试机制处理并发冲突
        let lastError = null;
     ...
     * 执行端口分配（在事务中）
     */
    async performAllocation(projectId, count) {
        return await this.prisma.$transaction(async (tx) => {
            ...
            if (availablePorts.length < count) {
                throw new PortAllocationError(`可用端口不足，需要${count}个，当前仅剩余${availablePorts.length}个端口`, 'INSUFFICIENT_PORTS', availablePorts.length);
            }
            // 5. 批量创建端口分配记录
            ...
            // 6. 构建返回结果
            const allocatedPorts = availablePorts.map(port => ({
                port,
                remark: '',
                allocatedAt: now
            }));
     ...
     * 获取项目已分配的端口
     */
    async getAllocatedPorts(projectId) {
     ...
     * 更新端口备注
     */
    async updatePortRemarks(projectId, updatedPorts) {
        // 验证端口是否属于该项目
        ...
        for (const updatedPort of updatedPorts) {
            ...
            if (!currentPort) {
                throw new PortAllocationError(`端口${updatedPort.port}不属于该项目`, 'INVALID_PORT_UPDATE');
            }
            // 检查端口号是否被修改
            if (updatedPort.port !== currentPort.port) {
                throw new PortAllocationError('不能修改端口号', 'INVALID_PORT_MODIFICATION');
            }
        }
